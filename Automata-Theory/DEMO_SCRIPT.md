...
| **[3:00 - Point out the transition path details]**<br>Hover over the subtext directly under the green **✓ Accepted** badge showing the state list: `Path: S0 → S1 → S3 → S4 → S6 → S8 → S8`. | "Right below the Accepted badge, it also prints out the exact state-by-state execution path our string followed." |
| **[3:12 - Input a rejecting string and validate]**<br>Click **+ Add Test Case** again, type an invalid string (like one that triggers a rejection), and click **Validate**. Point to the red **X Rejected** badge and its subtext `Rejected at S3`. | "Let's also test a string that doesn't match the regex. When we validate it, it gets flagged as Rejected, and the subtext tells us exactly which state the processing failed at—in this case, Rejected at S3." |
